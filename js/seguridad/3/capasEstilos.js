...
function crearCapaGanadores(año, tipoEleccion, cargo, nombreCapa, propiedadGanador = 'ganador') {
    // 1. Filtrar votos según parámetros
    ...
    // 2. Calcular ganadores por circuito
    ...
    datosFiltrados.forEach(entry => {
        ...
        if (!ganadoresPorCircuito[circuito]) {
            ganadoresPorCircuito[circuito] = {};
        }

        if (!ganadoresPorCircuito[circuito][agrupacion]) {
            ganadoresPorCircuito[circuito][agrupacion] = 0;
        }

        ganadoresPorCircuito[circuito][agrupacion] += cantidad;
    });

    // 3. Determinar el ganador en cada circuito
    ...
    // 4. Cargar features y asignar ganador
    ...
    const featuresConGanador = featuresCircuitos.map(f => {
        const idCircuito = f.get('name'); // Ajustar si el campo no es 'name'
    ...
    // 5. Crear capa con estilo personalizado
